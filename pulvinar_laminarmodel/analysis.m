...
function [pxx,fx,frequency,amplitudeA,amplitudeB,amplitudeC,mfr]=analysis(par,re,minfreq)
  ...
  [pxx,fx]=periodogram(restate',[],[],1./(par.dt));
  %mean(restate)
  ...
  newpxx=pxx(z);
  newfx=fx(z);
  %we rescale the power spectrum with respect to the baseline powerlaw fit:
  %the fit is:
  %p0=[0.0001 -2.];
  %p=fmins('powerlaw',p0,[],[],fx',pxx');
  %plot(fx',pxx',fx',p(1).*(fx').^(p(2)));
